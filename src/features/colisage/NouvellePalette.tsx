...
import { openAllPalette, openPalette } from 'config/rtk/rtkPalette';
import { Colis, Colisage, OpenPaletteProp, Palette, palette0, PaletteJson  } from 'tools/types';
...
function NouvelPalette({showModal,setShowModal,colis}:NouvellePaletteProps) {
    const PaletteToOpen:OpenPaletteProp = openAllPalette();
    const paletteJson: PaletteJson = PaletteToOpen.data;
    const Palettes: Palette[] = paletteJson.content;
  ...
  return (
    <Modal title={"Nouvelle Palette"} show={showModal} format={5} close={close}>{/*//@ts-ignore*/}
